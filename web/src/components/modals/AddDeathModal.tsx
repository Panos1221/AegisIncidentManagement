...
interface AddDeathModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (death: Death) => void
}

export default function AddDeathModal({ isOpen, onClose, onSave }: AddDeathModalProps) {
  const t = useTranslation()
  const agencyType = useUserAgencyType()
  const [formData, setFormData] = useState<Death>({
    name: '',
    type: 'civilian',
    description: ''
  })
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                rows={3}
                placeholder={t.description || 'Description of circumstances...'}
              />
            </div>
          </div>
